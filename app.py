...
def inject_user():
    if 'user_id' in session:
        res = supabase.table('users').select('profile_pic').eq('id', session['user_id']).execute()
...
def upload_profile_pic():
    try:
        ...
        if not f or not f.filename:
            return jsonify({'success': False, 'msg': 'File select karo.'})

        if not f.content_type.startswith('image/'):
            return jsonify({'success': False, 'msg': 'Sirf image file allowed.'})

        file_bytes = f.read()

        if len(file_bytes) > 5 * 1024 * 1024:
            return jsonify({'success': False, 'msg': 'File 5MB se chhoti honi chahiye.'})

        b64 = base64.b64encode(file_bytes).decode('utf-8')
        pic_url = f"data:{f.content_type};base64,{b64}"

        supabase.table('users').update({
            'profile_pic': pic_url
        }).eq('id', session['user_id']).execute()

        return jsonify({'success': True, 'pic': pic_url})
